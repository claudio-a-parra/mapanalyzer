...
class CompactMap:
    """Data Structure for fast access and manipulation of the MAP."""
    time_size = 1
    mem_size = 1
    base_addr = 0
    time_slice_size = 1
    space_slice_size = 1
    def __init__(self):
        self.time_space = [[] for _ in range(CompactMap.time_size)]
        self.space_time = [[] for _ in range(CompactMap.mem_size)]
        return
    
    def add_access(self, access):
        for offset in range(access.size):
            addr = access.addr - CompactMap.base_addr
            time = access.time
            self.time_space[time].append(addr+offset)
            self.space_time[addr+offset].append(time)
        return
    
    def get_spacially_sorted_time_window(self, time_last):
        # get time window [t, s]
        # (TODO: could be done by appending and trimming)
        time_first = max(0, time_last+1 - CompactMap.time_slice_size)
        win = []
        win_len = 0
        for t in range(time_last, time_first-1, -1):
            for s in reversed(self.time_space[t]):
                win.append((t,s))
                win_len += 1
                if win_len == CompactMap.time_slice_size:
                    break
            if win_len == CompactMap.time_slice_size:
                break
        win.sort(key=lambda x:(x[1],x[0])) # sort by space
        print(f'vert win   [:{time_last:2}] ({len(win)}) {win}')

        # compute deltas [dt, ds]
        deltas = win
        for a,b,i in zip(win[:-1],win[1:],range(len(win)-1)):
            deltas[i] = (abs(b[0]-a[0]), abs(b[1]-a[1]))
        del deltas[-1]
        print(f'vert deltas[:{time_last:2}] ({len(deltas)}) {deltas}')

        # compute loc [lt, ls]
        lt_fun = lambda dt: (CompactMap.time_slice_size - dt)\
            /CompactMap.time_slice_size
        ls_fun = lambda ds: (CompactMap.space_slice_size - ds)\
            /CompactMap.space_slice_size
        loc = [(lt_fun(dt),ls_fun(ds)) for dt,ds in deltas]
        print(f'vert local [:{time_last:2}] ({len(loc)}) {loc}')
        
        x = input()
        return win
    
    def get_temporally_sorted_space_window(self, space_last):
        win = []
        # get the left index of the window
        space_first = max(0, space_last+1 - CompactMap.space_slice_size)
        # get space window
        for s in range(space_first, space_last+1):
            for t in self.space_time[s]:
                win.append((t,s))
        # sort by time
        win.sort(key=lambda x:(x[0],x[1]))
        print(f'time win [:{space_last:2}] {win}')
        return win
...
class Locality(GenericInstrument):
    def __init__(self, instr_counter, cache_size, block_size, map_metadata,
        ...
        # Setup compact map class
        CompactMap.mem_size = map_metadata.mem_size
        CompactMap.base_addr = map_metadata.base_addr
        CompactMap.time_size = map_metadata.time_size
        CompactMap.time_slice_size = cache_size
        CompactMap.space_slice_size = block_size
    ...
    def register_access_DEPRECATED(self, access):
        if not self.enabled:
            return
    ...
    def register_access(self, access):
        if not self.enabled:
            return
        if access.thread not in self.buffer_traces:
            self.buffer_traces[access.thread] = CompactMap()
        self.buffer_traces[access.thread].add_access(access)
    ...
    def plot(self, axes, basename='locality', extent=None):
        for thread in self.buffer_traces:
            for i in range(CompactMap.time_size):
                twin = self.buffer_traces[thread].\
                    get_spacially_sorted_time_window(i)
            print()
            for i in range(CompactMap.mem_size):
                swin = self.buffer_traces[thread].\
                    get_temporally_sorted_space_window(i)
                print(f'spac win [:{i:2}]: {swin}')
